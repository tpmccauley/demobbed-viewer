...
dmED.createDrawGroupsAndTitles = function() {
  ...
  for (let ip = 0; ip < 2; ip++) {
    ...
    dmED.canvasesEmb(ip).append("g")
                        .attr("id", dmED.groupMuTrackIDs(ip));
...
dmED.displayEventInfo = function() {
  ...
  inputInputEvent.value = demobbed.event().id();
...
dmED.setNewCurrViewBounds = function(xyzmin, xyzmax) {
  ...
  if (newZoom < 1) alert("drawED.js::dmED.zoomToEvent()::Error: newZoom: " + newZoom + "!!!");
...
dmED.moveView = function(ip, dirLRUD) {
  ...
  if (!Utils.checkIP(ip, 3)) {
    alert("drawED.js::dmED.onMoveVert()::Error: ip is strange!!!: ip = " + ip + "!!!");
    return;
  }
...
dmED.updateCanvases = function() {
  ...
  if (dmED.canvasesMain(0) === null) dmED.initGraphics();
...
dmED.clearEvent = function(ip) {
  ...
  d3.select("#" + dmED.groupMuTrackIDs(ip))
    .selectAll('line')
    .remove();
  ...
  TTHitBoxes
    ...
    .attr('width', function(d) {
      ...
      if (dmED.zoom() > dmED.zoomFarViewMax()) {
        ...
        if (d.pe() > 200) return brd2 + dmED.hitTTdims(2);
        else return 0.005*brd2*d.pe() + dmED.hitTTdims(2);

      }
      else
        return dmED.hitTTdims(2);
...
dmED.drawHitsRPC = function(ip) {
  ...
  RPCHitBoxes
    ...
    .attr('y', function(d) {
      ...
      if (dmED.zoom() > dmED.zoomFarViewMax())
        return y - d.s()/(2*dmED.coefPixToCM());
      else
        return y - h2;
...
dmED.drawMuTrack = function(ip) {

  let MuTrackLine = d3.select("#" + dmED.groupMuTrackIDs(ip))
                      .selectAll('line')
                      .data(demobbed.event().trackMu());
  ...
  MuTrackLine
    ...
    .attr('x1', function(d) {
      ...
      let XYforZmin = d.Axy(ip)*dmED.currViewBounds().xyzMin[2] + d.Bxy(ip);

      if (XYforZmin < dmED.currViewBounds().xyzMin[ip])
        return dmED.scalesOfCanvEmb()[2]( (dmED.currViewBounds().xyzMin[ip] - d.Bxy(ip))/d.Axy(ip) );
      else
        return dmED.scalesOfCanvEmb()[2]( dmED.currViewBounds().xyzMin[2] );
    ...
    .attr('y1', function(d) {
      ...
      if (XYforZmin < dmED.currViewBounds().xyzMin[ip])
        return dmED.scalesOfCanvEmb()[ip]( dmED.currViewBounds().xyzMin[ip] );
      else
        return dmED.scalesOfCanvEmb()[ip]( XYforZmin );

    })
    .attr('x2', function(d) {

      let XYforZmax = d.Axy(ip)*dmED.currViewBounds().xyzMax[2] + d.Bxy(ip);

      if (XYforZmax > dmED.currViewBounds().xyzMax[ip])
        return dmED.scalesOfCanvEmb()[2]( (dmED.currViewBounds().xyzMax[ip] - d.Bxy(ip))/d.Axy(ip) );
      else
        return dmED.scalesOfCanvEmb()[2]( dmED.currViewBounds().xyzMax[2] );
    ...
    .attr('y2', function(d) {
      ...
      if (XYforZmax > dmED.currViewBounds().xyzMax[ip])
        return dmED.scalesOfCanvEmb()[ip]( dmED.currViewBounds().xyzMax[ip] );
      else
        return dmED.scalesOfCanvEmb()[ip]( XYforZmax );
