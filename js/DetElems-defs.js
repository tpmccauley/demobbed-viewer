class DetGeomElement { // Base class for geometry description of the OPERA detector components
  ...
  static checkID(jd) {
    ...
    if (!Utils.checkNumber(jd)) {
      alert("DetGeomElement::checkID()::Error: jd is not a number: jd = " + jd + "!!!");
      return false;
    }

    if ( (jd < 0) || (jd > 10000) ) {
      alert("DetGeomElement::checkID()::Error: jd is strange: jd = " + jd + "!!!");
      return false;
    }
  ...
  static checkXYMax(xymax) {
    ...
    if (!Utils.checkNumber(xymax)) {
      alert("DetGeomElement::checkXYMax()::Error: xymax is not a number: xymax = " + xymax + "!!!");
      return false;
    }

    if ( (xymax < DetCfg.globDetBounds().xyzMin[0]) || (xymax > DetCfg.globDetBounds().xyzMax[0]) ) {
      alert("DetGeomElement::checkXYMax()::Error: xymax is strange: xymax = " + xymax + "!!!");
      return false;
    }
  ...
  static checkZMin(zmin) {
    ...
    if (!Utils.checkNumber(zmin)) {
      alert("DetGeomElement::checkZMin()::Error: zmin is not a number: zmin = " + zmin + "!!!");
      return false;
    }

    if ( (zmin < DetCfg.globDetBounds().xyzMin[2]) || (zmin > DetCfg.globDetBounds().xyzMax[2]) ) {
      alert("DetGeomElement::checkZMin()::Error: zmin is strange: zmin = " + zmin + "!!!");
      return false;
    }
  ...
  static checkDim(dim) {
    ...
    if (!Utils.checkNumber(dim)) {
      alert("DetGeomElement::checkDim()::Error: dim is not a number: dim = " + dim + "!!!");
      return false;
    }

    if ( (dim <= 0) || (dim > 900) ) {
      alert("DetGeomElement::checkDim()::Error: dim is strange: dim = " + dim + "!!!");
      return false;
    }
  ...
  xyMax(ip) {
    ...
    if (!Utils.checkIP(ip)) {
      alert("DetGeomElement::xyMax()::Error: ip is wrong: ip = " + ip + "!!!");
      return;
    }
  ...
  dims(ip) {
    ...
    if (!Utils.checkIP(ip, 3)) {
      alert("DetGeomElement::dims()::Error: ip is wrong: ip = " + ip + "!!!");
      return;
    }
